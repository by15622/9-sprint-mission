...
/*현재 프로젝트 경로안에 "file-data-map" 라는 폴더를 만들고 그리고 Message class 이름(Message)만 가져와서 경로를 만든다
...
/* Resolvepath 메서드의 매개변수로 채널id 넣고 현재 프로젝트 경로/file-data-map/Message에 id.ser을 합쳐서 path에 저장한다
FileOutputStream은 이 프로젝트와 컴퓨터를 연결해서 path를 파일 형태로 바꾸고
ObjectOutputStream으로 해당 파일을 byte형식으로 변환하여 저장한다
만약 이 코드가 실행되는 중에 입출력하는 과정에서 오류가 발생할시 런타임 오류를 발생시킨다
다 실행된 후 리턴값으로 메세지를 반환한다
 */
    ...
    /* Message 타입의 messageNullable에 null을 대입하고 resolvePath메서드의 매개변수로 채널 id를 넣는다
    만약 저 경로가 존재할경우 FileOutputStream은 이 프로젝트와 컴퓨터를 연결해서 path를 파일 형태로 바꾸고
    ObjectOutputStream으로 byte 형식의 파일을 message 타입으로 변환하여 messageNullable에 대입한다
    만약 이 코드가 실행되는 중에 입출력하거나 클래스 없음 오류가 발생할시 런타임 오류를 발생시킨다
    다 실행된 후 Optional.ofNullable 명령어로 리턴값이 null이여도 오류가 발생하지 않게 messageNullable변수를 반환한다.
     */
    @Override
...
ObjectOutputStream으로 byte 형식의 파일을 Message 타입으로 변환하여 Message 객체를 담는 리스트에 담아 리턴한다
만약 이 코드가 실행되는 중에 입출력하거나 클래스 없음 오류가 발생할시 런타임 오류를 발생시킨다
 */
...
//findAll 메서드를 stream 방식으로 실행하는데 필터링을 message의 이름이 매개변수로 받은 이름이랑 같은 이름이면
//가장 첫번째 유저객체를 반환한다 리턴값(List)